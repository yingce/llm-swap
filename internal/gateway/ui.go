...
func (s *Server) recordAgentEvent(workerID string, event protocol.AgentEvent, receivedAt time.Time) {
	if event.Event == "" {
		return
	}
	cached := uiAgentEvent{
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .model-table { min-width: 1040px; table-layout: fixed; }
    .model-table th:nth-child(1), .model-table td:nth-child(1) { width: 18%; }
    .model-table th:nth-child(2), .model-table td:nth-child(2) { width: 14%; }
    .model-table th:nth-child(3), .model-table td:nth-child(3) { width: 24%; }
    .model-table th:nth-child(4), .model-table td:nth-child(4) { width: 16%; }
    .model-table th:nth-child(5), .model-table td:nth-child(5) { width: 28%; }
...
<body>
  ...
  <script>
    ...
    function renderModels(models) {
      ...
      document.getElementById("models").innerHTML = '<div class="table-wrap"><table class="model-table"><thead><tr><th>Model</th><th>Availability</th><th>Workers</th><th>Policy</th><th>Artifact</th></tr></thead><tbody>' + models.map((m) => {

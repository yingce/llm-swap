...
def deploy(ctx, host: str = DEFAULT_HOST, skip_tests: bool = False) -> None:
    ...
    remote_bash(
        ...
        docker run --rm \\
          ...
          "$GO_IMAGE" bash -lc 'mkdir -p dist && /usr/local/go/bin/go build -ldflags "-X llm-swap/internal/buildinfo.Commit=$LLMSWAP_BUILD_COMMIT -X llm-swap/internal/buildinfo.BuildTime=$LLMSWAP_BUILD_TIME" -o dist/llm-swap-gateway ./cmd/gateway'
